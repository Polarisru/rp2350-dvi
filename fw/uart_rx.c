...
static void report_error(uart_rx_t *ctx,
                         const char *raw_line,
                         const char *reason)
{
...
/* F:<color> */
static bool parse_cmd_fill(uart_rx_t *ctx,
                           char *payload,
...
/* f:<color> */
static bool parse_cmd_fill_8bit(uart_rx_t *ctx,
                                char *payload,
                                uart_rx_message_t *msg)
{
    uint16_t color;

    if (!parse_u16_strict(payload, &color) || (color > 255U))
    {
        report_error(ctx, payload, "f: invalid color (0..255)");
        return false;
    }

    msg->type = UART_RX_CMD_FILL_8BIT;
    msg->data.fill.color = (uint8_t)color;
...
/* R:<x1>:<y1>:<x2>:<y2>:<color> */
static bool parse_cmd_rect(uart_rx_t *ctx,
                           char *payload,
                           uart_rx_message_t *msg)
{
    char *p = payload;
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        if ((i < 4) && (tok == NULL))
        {
            report_error(ctx, payload, "R: too few fields");
            return false;
        }
        ...
        if (!parse_u16_strict(p, &fields[i]))
        {
            report_error(ctx, p, "R: invalid field");
            return false;
        }
    ...
    if (fields[4] > 7U)
    {
        report_error(ctx, payload, "R: invalid color (0..7)");
        return false;
    }
...
/* r:<x1>:<y1>:<x2>:<y2>:<color> */
static bool parse_cmd_rect_8bit(uart_rx_t *ctx,
                                char *payload,
                                uart_rx_message_t *msg)
{
    char *p = payload;
    ...
    if (fields[4] > 255U)
    {
        report_error(ctx, payload, "r: invalid color (0..255)");
        return false;
    }

    msg->type = UART_RX_CMD_RECT_8BIT;
    msg->data.rect.x1 = fields[0];
    msg->data.rect.y1 = fields[1];
    msg->data.rect.x2 = fields[2];
    msg->data.rect.y2 = fields[3];
...
static bool parse_cmd_box(uart_rx_t *ctx,
                          char *payload,
                          uart_rx_message_t *msg)
{
    char *p = payload;
    ...
    for (int i = 0; i < 6; i++)
    {
        ...
        if ((i < 5) && (tok == NULL))
        {
            report_error(ctx, payload, "B: too few fields");
            return false;
        }
        ...
        if (!parse_u16_strict(p, &fields[i]))
        {
            report_error(ctx, p, "B: invalid field");
            return false;
        }
    ...
    if (fields[4] > 255U)
    {
        report_error(ctx, payload, "B: invalid width (0..255)");
        return false;
    }

    if (fields[5] > 7U)
    {
        report_error(ctx, payload, "B: invalid color (0..7)");
        return false;
    }
...
/* b:<x1>:<y1>:<x2>:<y2>:<width>:<color> */
static bool parse_cmd_box_8bit(uart_rx_t *ctx,
                               char *payload,
                               uart_rx_message_t *msg)
{
    char *p = payload;
    ...
    if (fields[5] > 255U)
    {
        report_error(ctx, payload, "b: invalid color (0..255)");
        return false;
    }

    msg->type = UART_RX_CMD_BOX_8BIT;
    msg->data.box.x1 = fields[0];
    msg->data.box.y1 = fields[1];
    msg->data.box.x2 = fields[2];
    msg->data.box.y2 = fields[3];
...
static bool parse_cmd_text(uart_rx_t *ctx,
                           char *payload,
                           uart_rx_message_t *msg)
{
    char *p = payload;
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        if (tok == NULL)
        {
            report_error(ctx, payload, "T: too few fields");
            return false;
        }

        *tok = '\0';

        if (!parse_u16_strict(p, &nums[i]))
        {
            report_error(ctx, p, "T: invalid field");
            return false;
        }
    ...
    if (nums[2] > 7U)
    {
        report_error(ctx, payload, "T: invalid color (0..7)");
        return false;
    }

    if (nums[3] > 2U)
    {
        report_error(ctx, payload, "T: invalid font (0..2)");
        return false;
    }
    ...
    msg->data.draw_text.text = p;

    return true;
}

/* t:<x>:<y>:<color>:<font>:<text> */
static bool parse_cmd_text_8bit(uart_rx_t *ctx,
                                char *payload,
                                uart_rx_message_t *msg)
{
    char *p = payload;
    ...
    if (nums[2] > 255U)
    {
        report_error(ctx, payload, "t: invalid color (0..255)");
        return false;
    }
    ...
    msg->type = UART_RX_CMD_TEXT_8BIT;
    msg->data.draw_text.x = nums[0];
    msg->data.draw_text.y = nums[1];
    msg->data.draw_text.color = (uint8_t)nums[2];
    msg->data.draw_text.font = (uint8_t)nums[3];
...
static void parse_line(uart_rx_t *ctx, char *line)
{
    ...
    if (line[1] == ':')
    {
        ...
        {
        case 'F':
            ok = parse_cmd_fill(ctx, payload, &msg);
            break;

        case 'f':
            ok = parse_cmd_fill_8bit(ctx, payload, &msg);
            break;

        case 'R':
            ok = parse_cmd_rect(ctx, payload, &msg);
            break;

        case 'r':
            ok = parse_cmd_rect_8bit(ctx, payload, &msg);
            break;

        case 'B':
            ok = parse_cmd_box(ctx, payload, &msg);
            break;

        case 'b':
            ok = parse_cmd_box_8bit(ctx, payload, &msg);
            break;

        case 'T':
            ok = parse_cmd_text(ctx, payload, &msg);
            break;

        case 't':
            ok = parse_cmd_text_8bit(ctx, payload, &msg);
            break;
    ...
    *first_colon = '\0';
    ...
    if (line_index == 0U)
    {
        ...
        if (second_colon == NULL)
        {
            report_error(ctx,
                         payload,
                         "header needs number_of_lines:font");
            return;
        }
        ...
        *second_colon = '\0';
        ...
        if (number_of_lines > 255U)
        {
            report_error(ctx, payload, "invalid number_of_lines (0..255)");
            return;
        }

        if (font > 2U)
        {
            report_error(ctx, second_colon + 1, "invalid font (0..2)");
            return;
        }

        msg.type = UART_RX_LINE_TYPE_HEADER;
        msg.data.header.number_of_lines = (uint8_t)number_of_lines;
    ...
    else
    {
        ...
        *second_colon = '\0';
        ...
        if (!parse_u16_strict(second_colon + 1, &color) || (color > 7U))
        {
            report_error(ctx, second_colon + 1, "invalid color (0..7)");
            return;
        }

        msg.type = UART_RX_LINE_TYPE_TEXT;
        msg.data.text.text = payload;
        msg.data.text.color = (uint8_t)color;
    }
